...
@Component({
  ...
  template: `
    <div class="report-container">
      <h2>รายงาน AP Aging (เจ้าหนี้ตามอายุหนี้)</h2>

      <form [formGroup]="filterForm" (ngSubmit)="applyFilter()" class="filter-form">
        <div class="filter-fields">
          <div class="field">
            <label for="vendorId">รหัสผู้ขาย</label>
            <input id="vendorId" type="text" formControlName="vendorId" placeholder="กรอกรหัสผู้ขาย" />
          </div>
          <div class="field">
      ...
      @if (summary(); as s) {
        <div class="summary-bar">
          <div class="summary-item">
            <span class="summary-label">ยอดเจ้าหนี้คงค้างรวม:</span>
            <span class="summary-value">{{ s.totalOpen | number:'1.2-2' }} THB</span>
          </div>
          <div class="summary-item">
            <span class="summary-label">จำนวนผู้ขาย:</span>
            <span class="summary-value">{{ s.totalVendors }}</span>
          </div>
        </div>
      }
      ...
      @if (!loading() && !error() && data().length > 0) {
        <div class="table-wrapper">
          <table class="report-table" aria-label="AP Aging Report">
            <thead>
              <tr>
            ...
            <tbody>
              @for (row of data(); track row.vendorId) {
                <tr>
      ...
      @if (!loading() && !error() && data().length === 0) {
        <div class="empty-state"><p>ไม่พบข้อมูล AP Aging</p></div>
      }
    </div>
  `,
  styles: [`
...
export class ApAgingComponent implements OnInit {
  ...
  readonly data = signal<APAgingView[]>([]);
  readonly summary = signal<APAgingSummary | null>(null);
  ...
  resetFilter(): void {
    this.filterForm.reset({ vendorId: '', asOfDate: '' });
    this.currentPage = 1;
    this.loadData();
  }
  ...
  loadData(): void {
    ...
    const filter: APAgingFilter = {
      page: this.currentPage,
      limit: this.pageSize,
    };

    if (formValue.vendorId) filter.vendorId = formValue.vendorId;
